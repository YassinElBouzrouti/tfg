...
- El frontend y la API se despliegan en el mismo proyecto de Vercel.
- La configuracion esta en `vercel.json` y expone la API en `/api`.
- En produccion, el frontend usa `/api` automaticamente (no necesita `VITE_API_URL` si todo va en el mismo dominio).
...
- `CLIENT_URL` (opcional, para origen explicito)
